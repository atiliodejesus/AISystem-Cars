...
public class RoadManagerSystem : MonoBehaviour
{
  ...
  private void DrawGizmos(bool selected)
  {
    ...
    if (Waypoints.Length > 1)
    {
      ...
      Gizmos.color = new Color(0.8f, 0.9f, 0.8f, 1);
...
public class WaypointListDrawing : PropertyDrawer
{
  ...
  public override void OnGUI(Rect position, SerializedProperty property, GUIContent label)
  {
    ...
    if (GUI.Button(addAllButtonRect, "Adicionar TrackPoints"))
    {
    ...
    if (GUI.Button(renameButtonRect, "Ordenar TrackPoints"))
    {
      var circuit = property.FindPropertyRelative("circuit").objectReferenceValue as RoadManagerSystem;
      int n = 0;
      foreach (Transform child in circuit.waypointList.items)
      {
        child.name = "Trackpoint " + (n++).ToString("000");
      }
    }
    y += lineHeight + spacing;
